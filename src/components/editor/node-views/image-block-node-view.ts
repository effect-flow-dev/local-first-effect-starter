  import {
    ...
    export class ImageBlockNodeView extends LitElement {
      ...
      override willUpdate(changedProperties: PropertyValues<this>) {
        ...
        // ✅ FIXED: Clean up the local blob once the remote URL is ready.
        // We detect the transition from having an uploadId to not having one while a URL is present.
        const prevUploadId = changedProperties.get("uploadId");
        const isFinishingUpload = (prevUploadId !== undefined && prevUploadId !== "") && !this.uploadId && !!this.url;

        if (isFinishingUpload) {
            runClientUnscoped(clientLog("info", "[ImageNode] Finalizing upload transition. Clearing blob."));
            this._cleanup();
            return; 
        }

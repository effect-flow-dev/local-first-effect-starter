...
export class ImageBlockNodeView extends LitElement {
  ...
  override willUpdate(changedProperties: PropertyValues<this>) {
    ...
    if (changedProperties.has("url") || changedProperties.has("uploadId")) {
      if (
        ...
        // Re-init if props changed substantially
        this._cleanup();
        this._init();
      }
    }
  ...
  private _init() {
    // ✅ FIX: Prefer loading local blob if uploadId exists (Binary GC / Read-Through)
    // Even if we have a URL, we want to check IDB first to save bandwidth
    // and use the local copy.
    if (this.uploadId) {
      this._loadBlob();
    }
    
    // Only poll if we are actively uploading (no URL yet)
    if (this.uploadId && !this.url) {
      this._startPolling();
    }
  }
  ...
  private _loadBlob() {
    ...
    runClientUnscoped(
      loadEffect.pipe(
        ...
        Effect.catchAll((e) =>
          Effect.sync(() => {
            // ✅ FIX: Only mark as error if we don't have a fallback URL
            if (!self.url && !self._localBlobUrl) {
              console.warn(
            ...
            // If we have a URL, we simply fail silently on local blob load
            // and let render() use the URL.
          }),
        ),
      ),
    );
  ...
  override render() {
    ...
    // Only consider it a fatal error if we have NO url and status is error

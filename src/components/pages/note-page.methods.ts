...
/**
 * Generates a causal stamp for mutations.
 */
const getCausalStamp = () => Effect.gen(function*() {
    const hlc = yield* HlcService;
    return {
        hlcTimestamp: yield* hlc.getNextHlc(),
...
export const handleTaskUpdate = (
  component: NotePage,
  e: CustomEvent<{ blockId: BlockId; isComplete: boolean; version: number }>,
) => {
  runClientUnscoped(Effect.gen(function*() {
      const stamp = yield* getCausalStamp();
      component.dispatch({ 
          type: "UPDATE_TASK_START", 
          payload: { ...e.detail, ...stamp } 
      });
  }));
};
...
export const handleBlockUpdateMethod = (
    component: NotePage,
    blockId: BlockId,

...
// --- HELPER: Shared Note Update Logic ---
export const _applyNoteUpdate = (
  ...
  globalVersion: number
    ...
    // 1. Construct dynamic update payload
    ...
    if (updateResult.length === 0) {
        yield* Effect.logWarning(`[_applyNoteUpdate] Note ${args.id} not found.`);
        return;
    }

    // ✅ OPTIMIZATION: Only shred blocks if content was actually provided
    if (args.content) {
        const incomingBlocksDTO = parseContentToBlocks(
          args.id,
          userId,
          args.content,
        );
        
        const incomingIds = incomingBlocksDTO
          .map((b) => b.id)
          .filter((id): id is BlockId => id !== undefined);

        yield* Effect.tryPromise({
            try: async () => {
              // Delete removed blocks (Tombstones)
              ...
              // Upsert new/updated blocks
              if (incomingBlocksDTO.length > 0) {
...
export const handleUpdateNote = (
  ...
  Effect.gen(function* () {
    yield* Effect.logInfo(`[handleUpdateNote] Update noteId: ${args.id}`);
...
export const handleRevertNote = (
  ...
  Effect.gen(function* () {
    yield* Effect.logInfo(`[handleRevertNote] Reverting note ${args.noteId} to history ${args.historyId}`);
    ...
    const existingNote = yield* Effect.tryPromise({
        try: () =>
          db
            .selectFrom("note")
            .select("id")
            .where("title", "=", args.targetSnapshot.title)
    ...
    if (existingNote) {
        titleToRestore = `${titleToRestore} (Restored)`;
    }
